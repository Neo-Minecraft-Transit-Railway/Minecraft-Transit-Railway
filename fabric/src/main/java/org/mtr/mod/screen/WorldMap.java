			...
			for (int j = -radius; j < radius + 1; j++) {
				...
				for (int k = 0; k < CHUNK_SIZE; k++) {
					for (int l = 0; l < CHUNK_SIZE; l++) {
						...
						if (nativeImage != null) {
							// 1.21 setPixelColor maps to RGBA setPixel; use setPixelABGR with ABGR packed color
							nativeImage.setPixelABGR(k, l, convertColorABGR(divideColorRGB(world.getBlockState(finalPos).getBlock().getDefaultMapColor().getColorMapped(), mapOverlayMode == MapOverlayMode.TOP_VIEW ? 1.5 : 1 + ((15 - lightLevel) / LIGHT_DIM_FACTOR))));
						}
					}
				}
