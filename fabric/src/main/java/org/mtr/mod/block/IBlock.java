...
public interface IBlock {
	...
	static ActionResult checkHoldingBrush(World world, PlayerEntity player, Runnable callbackBrush, @Nullable Runnable callbackNoBrush) {
		// Vanilla "刷子" is also accepted: players commonly pick it instead of MTR 核心「刷」.
		return checkHoldingItem(world, player, item -> callbackBrush.run(), callbackNoBrush, Items.BRUSH.get(), org.mtr.mapping.holder.Items.getBrushMapped());
	}

	static boolean isHoldingItem(PlayerEntity player, Item item) {
		if (player.isHolding(item)) {
			return true;
		}
		// 1.21 ItemStack.is(Item) can miss wrapped holders; compare the raw item too.
		final ItemStack main = player.getMainHandStack();
		final ItemStack off = player.getOffHandStack();
		return main.getItem().data == item.data || off.getItem().data == item.data;
	}
