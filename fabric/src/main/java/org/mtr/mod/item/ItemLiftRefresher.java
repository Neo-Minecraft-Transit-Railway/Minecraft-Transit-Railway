...
public class ItemLiftRefresher extends ItemExtension implements DirectionHelper {
	...
	public ActionResult useOnBlock2(ItemUsageContext context) {
		...
		final ObjectArrayList<LiftFloor> liftFloors = collectFloors(world, context.getBlockPos(), playerEntity);
		if (liftFloors == null) {
			return ActionResult.getFailMapped();
		}

		final Lift lift = createLift(liftFloors);
		if (world.isClient()) {
			ItemLiftRefresherClient.open(lift);
			return ActionResult.SUCCESS;
		}

		Init.sendMessageC2S(OperationProcessor.GENERATE_BY_LIFT, ServerWorld.cast(world).getServer(), new World(world.data), lift, null, null);
		Init.REGISTRY.sendPacketToClient(ServerPlayerEntity.cast(playerEntity), new PacketOpenLiftCustomizationScreen(Init.positionToBlockPos(liftFloors.get(0).getPosition()), Utilities.getJsonObjectFromData(lift).toString()));
		return new ActionResult(ActionResult.getSuccessServerMapped());
	}

	@Nullable
	private static ObjectArrayList<LiftFloor> collectFloors(World world, BlockPos blockPos, PlayerEntity playerEntity) {
		...
		return needsReverse ? reverseList(liftFloors) : liftFloors;
	}

	private static Lift createLift(ObjectArrayList<LiftFloor> liftFloors) {
