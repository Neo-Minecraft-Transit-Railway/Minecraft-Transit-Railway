	...
	public static void render(long millisElapsed, Vector3d cameraShakeOffset) {
		...
		MinecraftClientData.getInstance().liftWrapperList.values().forEach(liftWrapper -> {
			...
			if (liftWrapper.shouldRender) {
				...
				if (lift.hasCoolDown()) {
					doorway1Open = RenderVehicleHelper.canOpenDoors(doorway1, absolutePositionAndRotation, Math.min(lift.getDoorValue(), LIFT_DOOR_VALUE), lift.getHeight());
					doorway2Open = lift.getIsDoubleSided() && RenderVehicleHelper.canOpenDoors(doorway2, absolutePositionAndRotation, Math.min(lift.getDoorValue(), LIFT_DOOR_VALUE), lift.getHeight());
					if (doorway1Open) {
