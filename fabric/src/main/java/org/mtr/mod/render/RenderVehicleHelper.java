...
public class RenderVehicleHelper {
	...
	public static boolean canOpenDoors(Box doorway, PositionAndRotation positionAndRotation, double doorValue) {
		return canOpenDoors(doorway, positionAndRotation, doorValue, 0);
	}

	/**
	 * @param carHeight lifts only: open landing doors that overlap the car's current Y span.
	 *                  Trains pass {@code 0} and keep the 1.20 Y±2 scan with no floor filter.
	 */
	public static boolean canOpenDoors(Box doorway, PositionAndRotation positionAndRotation, double doorValue, double carHeight) {
		...
		final double carMinY = positionAndRotation.position.y;
		final double carMaxY = carHeight > 0 ? carMinY + carHeight : Double.POSITIVE_INFINITY;
			...
			for (double checkY = minY - CHECK_DOOR_RADIUS_Y; checkY <= maxY + CHECK_DOOR_RADIUS_Y; checkY++) {
				for (double checkZ = minZ - CHECK_DOOR_RADIUS_XZ; checkZ <= maxZ + CHECK_DOOR_RADIUS_XZ; checkZ++) {
					...
					if (block.data instanceof PlatformHelper) {
						...
						if (carHeight > 0) {
							final int doorY = checkPos.getY();
							if (doorY >= carMaxY || doorY + 1 <= carMinY) {
								continue;
							}
						}
						canOpenDoors = true;
